...
def test_pad(model, L, modulo=16, sf=1):
    ...
    L = paddle.nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))(L)
    E = model(L)
    E = E[..., :h*sf, :w*sf]
    return E
...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    h, w = L.size()[-2:]
    if h*w <= min_size**2:
        L = paddle.nn.ReplicationPad2d((0, int(np.ceil(w/modulo)*modulo-w), 0, int(np.ceil(h/modulo)*modulo-h)))(L)
        E = model(L)
        E = E[..., :h*sf, :w*sf]
    else:
...
def test_x8(model, L, modulo=1, sf=1):
    ...
    output_cat = paddle.stack(E_list, dim=0)
...
def describe_model(model):
    if isinstance(model, paddle.nn.DataParallel):
        model = model.module
    msg = '\n'
