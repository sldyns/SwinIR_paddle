...
def splits(a, sf):
    ...
    '''
    b = paddle.stack(paddle.chunk(a, sf, dim=2), dim=5)
    b = paddle.cat(paddle.chunk(b, sf, dim=3), dim=5)
    return b


def c2c(x):
    return paddle.from_numpy(np.stack([np.float32(x.real), np.float32(x.imag)], axis=-1))


def r2c(x):
    return paddle.stack([x, paddle.zeros_like(x)], -1)
...
def cdiv(x, y):
    ...
    c, d = y[..., 0], y[..., 1]
    cd2 = c**2 + d**2
    return paddle.stack([(a*c+b*d)/cd2, (b*c-a*d)/cd2], -1)


def csum(x, y):
    return paddle.stack([x[..., 0] + y, x[..., 1]], -1)


def cabs(x):
    return paddle.pow(x[..., 0]**2+x[..., 1]**2, 0.5)
...
def cmul(t1, t2):
    ...
    return paddle.stack([real1 * real2 - imag1 * imag2, real1 * imag2 + imag1 * real2], dim=-1)
...
def rfft(t):
    return paddle.rfft(t, 2, onesided=False)


def irfft(t):
    return paddle.irfft(t, 2, onesided=False)


def fft(t):
    return paddle.fft(t, 2)


def ifft(t):
    return paddle.ifft(t, 2)
...
def p2o(psf, shape):
    ...
    Returns:
        otf: NxCxHxWx2
    '''
    otf = paddle.zeros(psf.shape[:-2] + shape).type_as(psf)
    otf[...,:psf.shape[2],:psf.shape[3]].copy_(psf)
    for axis, axis_size in enumerate(psf.shape[2:]):
        otf = paddle.roll(otf, -int(axis_size / 2), dims=axis+2)
    otf = paddle.rfft(otf, 2, onesided=False)
    n_ops = paddle.sum(paddle.tensor(psf.shape).type_as(psf) * paddle.log2(paddle.tensor(psf.shape).type_as(psf)))
    otf[...,1][paddle.abs(otf[...,1])<n_ops*2.22e-16] = paddle.tensor(0).type_as(psf)
    return otf
...
def INVLS_pypaddle(FB, FBC, F2B, FR, tau, sf=2):
    '''
    FB: NxCxWxHx2
    F2B: NxCxWxHx2
    ...
    '''
    x1 = cmul(FB, FR)
    FBR = paddle.mean(splits(x1, sf), dim=-1, keepdim=False)
    invW = paddle.mean(splits(F2B, sf), dim=-1, keepdim=False)
    invWBR = cdiv(FBR, csum(invW, tau))
    FCBinvWBR = cmul(FBC, invWBR.repeat(1,1,sf,sf,1))
    FX = (FR-FCBinvWBR)/tau
    Xest = paddle.irfft(FX, 2, onesided=False)
    return Xest
...
def upsample(x, sf=3, center=False):
    ...
    z = paddle.zeros((x.shape[0], x.shape[1], x.shape[2]*sf, x.shape[3]*sf)).type_as(x)
...
def circular_pad(x, pad):
    ...
    '''
    x = paddle.cat([x, x[:, :, 0:pad, :]], dim=2)
    x = paddle.cat([x, x[:, :, :, 0:pad]], dim=3)
    x = paddle.cat([x[:, :, -2 * pad:-pad, :], x], dim=2)
    x = paddle.cat([x[:, :, :, -2 * pad:-pad], x], dim=3)
    return x
...
def dim_pad_circular(input, padding, dimension):
    ...
    input = paddle.cat([input, input[[slice(None)] * (dimension - 1) +
                      [slice(0, padding)]]], dim=dimension - 1)
    input = paddle.cat([input[[slice(None)] * (dimension - 1) +
...
def imfilter(x, k):
    ...
    x = paddle.nn.functional.Conv2D(x, k, groups=x.shape[1])
    return x
...
def interpolation_down(x, sf, center=False):
    ...
    if center:
        start = paddle.tensor((sf-1)//2)
        mask[..., start::sf, start::sf] = paddle.tensor(1).type_as(x)
        LR = x[..., start::sf, start::sf]
    else:
        mask[..., ::sf, ::sf] = paddle.tensor(1).type_as(x)
        LR = x[..., ::sf, ::sf]
    y = x.mul(mask)
