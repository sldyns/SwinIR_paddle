...
# 03/Mar/2019
# --------------------------------------------
# https://github.com/twhui/SRGAN-pypaddle
...
def make_grid(tensor, nrow=8, padding=2,
    ...
    if normalize is True:
        ...
        def norm_ip(img, min, max):
            img = paddle.clip(img,min=min, max=max)
            img = img - min.devide(max - min + 1e-5)
...
# convert uint to 4-dimensional paddle tensor
def uint2tensor4(img):
    if img.ndim == 2:
        img = np.expand_dims(img, axis=2)
    return paddle.Tensor(np.ascontiousarray(img,dtype=np.float32)).transpose([2, 0, 1]).unsqueeze(0) / 255.


# convert uint to 3-dimensional paddle tensor
def uint2tensor3(img):
    if img.ndim == 2:
        img = np.expand_dims(img, axis=2)
    return paddle.Tensor(np.ascontiguousarray(img,dtype=np.float32)).transpose([2, 0, 1]) / 255.


# convert 2/3/4-dimensional paddle tensor to uint
def tensor2uint(img):
    img = np.clip(img.numpy().squeeze(),0, 1)
    if img.ndim == 3:
...
def single2tensor3(img):
    return paddle.Tensor(np.ascontiguousarray(img,dtype=np.float32)).transpose([2, 0, 1])


# convert single (HxWxC) to 4-dimensional paddle tensor
def single2tensor4(img):
    return paddle.Tensor(np.ascontiguousarray(img,dtype=np.float32)).transpose([2, 0, 1]).unsqueeze(0)


# convert paddle tensor to single
...
def single2tensor5(img):
    return paddle.Tensor(np.ascontiguousarray(img)).transpose([2, 0, 1, 3]).unsqueeze(0)


def single32tensor5(img):
    return paddle.Tensor(np.ascontiguousarray(img)).unsqueeze(0).unsqueeze(0)


def single42tensor4(img):
    return paddle.Tensor(np.ascontiguousarray(img)).transpose([2, 0, 1, 3]).float()


# from skimage.io import imread, imsave
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1)):
    '''
    Converts a paddle Tensor into an image Numpy array of BGR channel order
    Input: 4D(B,(3/1),H,W), 3D(C,H,W), or 2D(H,W), any range, RGB channel order
    Output: 3D(H,W,C) or 2D(H,W), [0,255], np.uint8 (default)
    '''
    tensor = paddle.clip(tensor.squeeze().float(), min_max[0], min_max[1])  # squeeze first, then clamp
...
def augment_img_tensor(img, mode=0):
    ...
    img_tensor = paddle.Tensor(np.ascontiguousarray(img_np))
    if len(img_size) == 3:
        img_tensor = img_tensor.transpose([2, 0, 1])
    elif len(img_size) == 4:
        img_tensor = img_tensor.transpose([3, 2, 0, 1])
...
def calculate_psnrb(img1, img2, border=0):
    ...
    img1 = paddle.Tensor(img1).transpose([2, 0, 1]).unsqueeze(0) / 255.
    img2 = paddle.Tensor(img2).transpose([2, 0, 1]).unsqueeze(0) / 255.
...
'''
# --------------------------------------------
# matlab's bicubic imresize (numpy and paddle) [0, 1]
# --------------------------------------------
'''
...
def imresize(img, scale, antialiasing=True):
    ...
    # input: img: pypaddle tensor, CHW or HW [0,1]
    ...
    # process H dimension
    # symmetric copying
    img_aug = paddle.Tensor([in_C, in_H + sym_len_Hs + sym_len_He, in_W])
    ...
    out_1 = paddle.Tensor([in_C, out_H, in_W])
    ...
    # process W dimension
    # symmetric copying
    out_1_aug = paddle.Tensor([in_C, out_H, in_W + sym_len_Ws + sym_len_We])
    ...
    out_2 = paddle.Tensor([in_C, out_H, out_W])
...
def imresize_np(img, scale, antialiasing=True):
    ...
    # process H dimension
    # symmetric copying
    img_aug = paddle.Tensor([in_H + sym_len_Hs + sym_len_He, in_W, in_C])
    img_aug.narrow(0, sym_len_Hs, in_H).copy_(img)

    sym_patch = img[:sym_len_Hs, :, :]
    inv_idx = paddle.arange(sym_patch.shape[0] - 1, -1, -1).long()
    ...
    out_1 = paddle.Tensor([out_H, in_W, in_C])
    kernel_width = weights_H.shape[1]
    for i in range(out_H):
        idx = int(indices_H[i][0])
        for j in range(out_C):
    ...
    # process W dimension
    # symmetric copying
    out_1_aug = paddle.Tensor([out_H, in_W + sym_len_Ws + sym_len_We, in_C])
    ...
    out_2 = paddle.FloatTensor(out_H, out_W, in_C)
    kernel_width = weights_W.shape[1]
    for i in range(out_W):
        idx = int(indices_W[i][0])
        for j in range(out_C):
