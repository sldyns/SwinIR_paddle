...
"""
# --------------------------------------------
# Convert matconvnet SimpleNN model into pypaddle model
# --------------------------------------------
...
def weights2tensor(x, squeeze=False, in_features=None, out_features=None):
    """Modified version of https://github.com/albanie/pypaddle-mcn
    Adjust memory layout and load weights as paddle tensor
    Args:
        ...
        squeeze (bool) [False]: whether to squeeze the tensor (i.e. remove
           ...
           pypaddle layout (C_out, C_in, H, W), if the shape is (A, B, 1, 1)
    ...
    Returns:
        paddle.tensor: a permuted sets of weights, matching the pypaddle layout
        convention
    """
    if x.ndim == 4:
...
#            x=x[[0,4,8,12,1,5,9,13,2,6,10,14,3,7,11,15,  0+16,4+16,8+16,12+16,1+16,5+16,9+16,13+16,2+16,6+16,10+16,14+16,3+16,7+16,11+16,15+16,  0+32,4+32,8+32,12+32,1+32,5+32,9+32,13+32,2+32,6+32,10+32,14+32,3+32,7+32,11+32,15+32],:,:,:]
    ...
    if squeeze:
        ...
        x = np.squeeze(x)
    return paddle.from_numpy(np.ascontiguousarray(x))
...
#    
    ...
    paddle.save(mat_net, 'model_zoo/modelcolor.pdparams')
...
#
#    paddle.save(mat_net, 'E:/pypaddle/KAIR_ongoing/model_zoo/ffdnet_gray_clip.pth')
#    
#    
#
#    crt_net = paddle.load('E:/pypaddle/KAIR_ongoing/model_zoo/imdn_x4.pth')
#    def show_kv(net):
...
#        mat_net[key] = param2
#    paddle.save(mat_net, 'model_zoo/imdn_x4_1.pth') 
#        

#    net_old = paddle.load('net_old.pth')
#    def show_kv(net):
#        for k, v in net.items():
#            print(k)
#
...
#    paddle.save(net_new, 'net_new.pth') 
