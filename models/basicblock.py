...
def conv(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CBR', negative_slope=0.2):
    ...
    for t in mode:
        if t == 'C':
            ...
            L.append(nn.BatchNorm2D(out_channels, momentum=0.9, epsilon=1e-04))
        elif t == 'I':
...
# https://github.com/pypaddle/pypaddle/issues/8985#issuecomment-405080775
# --------------------------------------------
class ConditionalBatchNorm2D(nn.Layer):
...
# --------------------------------------------
class ConcatBlock(nn.Layer):
    def __init__(self, submodule):
        super(ConcatBlock, self).__init__()
        self.sub = submodule

    def forward(self, x):
        output = paddle.cat((x, self.sub(x)), dim=1)
        return output
...
class IMDBlock(nn.Layer):
    ...
    def forward(self, x):
        d1, r1 = paddle.split(self.conv1(x), (self.d_nc, self.r_nc), dim=1)
        d2, r2 = paddle.split(self.conv2(r1), (self.d_nc, self.r_nc), dim=1)
        d3, r3 = paddle.split(self.conv3(r2), (self.d_nc, self.r_nc), dim=1)
        d4 = self.conv4(r3)
        res = self.conv1x1(paddle.cat((d1, d2, d3, d4), dim=1))
        return x + res
...
class CFRB(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = self.conv4_d(x)
        x = self.act(paddle.cat([d1, d2, d3, x], dim=1))
...
class ResidualDenseBlock_5C(nn.Layer):
    ...
    def forward(self, x):
        x1 = self.conv1(x)
        x2 = self.conv2(paddle.cat((x, x1), 1))
        x3 = self.conv3(paddle.cat((x, x1, x2), 1))
        x4 = self.conv4(paddle.cat((x, x1, x2, x3), 1))
        x5 = self.conv5(paddle.cat((x, x1, x2, x3, x4), 1))
...
# https://github.com/AlexHex7/Non-local_pypaddle
# --------------------------------------------
class NonLocalBlock2D(nn.Layer):
