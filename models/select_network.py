...
def define_F(opt, use_bn=False):
    ...
    # pypaddle pretrained VGG19-54, before ReLU.
    if use_bn:
        feature_layer = 49
    else:
        feature_layer = 34
...
def init_weights(net, init_type='xavier_uniform', init_bn_type='uniform', gain=1):
    ...
    def init_fn(m, init_type='xavier_uniform', init_bn_type='uniform', gain=1):
        ...
        if classname.find('Conv') != -1 or classname.find('Linear') != -1:
            ...
            if init_type == 'normal':
                m.weight_attr = ParamAttr(initializer=initializer.Normal(0, 0.1))
                m.weight_attr = paddle.clip(m.weight_attr, -1, 1) * gain

            elif init_type == 'uniform':
                m.weight_attr = ParamAttr(initializer=initializer.Uniform(m.weight.data, -0.2, 0.2))
                m.weight_attr = paddle.clip(m.weight_attr, -1, 1) * gain

            elif init_type == 'xavier_normal':
                m.weight_attr = ParamAttr(initializer=initializer.XavierNormal(m.weight.data))
                m.weight_attr = paddle.clip(m.weight_attr, -1, 1)

            elif init_type == 'xavier_uniform':
                m.weight_attr = ParamAttr(initializer=initializer.XavierUniform(m.weight.data))

            elif init_type == 'kaiming_normal':
                m.weight_attr = ParamAttr(initializer=initializer.KaimingNormal(m.weight.data))
                m.weight_attr = paddle.clip(m.weight_attr, -1, 1)

            elif init_type == 'kaiming_uniform':
                m.weight_attr = ParamAttr(initializer=initializer.KaimingUniform(m.weight.data))
                m.weight_attr = paddle.clip(m.weight_attr, -1, 1) * gain
            ...
            if init_bn_type == 'uniform':  # preferred
                if m.affine:
                    m.weight_attr = ParamAttr(initializer=initializer.Uniform(0.1, 1.0))
                    m.bias_attr = ParamAttr(initializer=initializer.Constant(0.0))
            elif init_bn_type == 'constant':
                if m.affine:
                    m.weight_attr = ParamAttr(initializer=initializer.Constant(1.0))
                    m.bias_attr = ParamAttr(initializer=initializer.Constant(0.0))
            else:
