...
def splits(a, sf):
    ...
    '''
    b = paddle.stack(paddle.chunk(a, sf, dim=2), dim=4)
    b = paddle.cat(paddle.chunk(b, sf, dim=3), dim=4)
    return b
...
def p2o(psf, shape):
    ...
    otf = paddle.fft.fftn(otf, dim=(-2,-1))
...
class DataNet(nn.Layer):
    ...
    def forward(self, x, FB, FBC, F2B, FBFy, alpha, sf):
        ...
        FR = FBFy + paddle.fft.fftn(alpha*x, dim=(-2,-1))
        x1 = FB.mul(FR)
        ...
        Xest = paddle.real(paddle.fft.ifftn(FX, dim=(-2,-1)))
...
class USRNet(nn.Layer):
    ...
    def forward(self, x, k, sf, sigma):
        ...
        FB = p2o(k, (w*sf, h*sf))
        FBC = paddle.conj(FB)
        F2B = paddle.pow(paddle.abs(FB), 2)
        STy = upsample(x, sf=sf)
        FBFy = FBC*paddle.fft.fftn(STy, dim=(-2,-1))
