...
from paddle.nn.Layers.utils import _pair, _single
import numpy as np
...
class ModulatedDeformConv(nn.Layer):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(paddle.Tensor(out_channels, in_channels // groups, *self.kernel_size))
        if bias:
            self.bias = nn.Parameter(paddle.Tensor(out_channels))
        else:
...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    #     o1, o2, mask = paddle.chunk(out, 3, dim=1)
    #     offset = paddle.cat((o1, o2), dim=1)
    #     mask = paddle.sigmoid(mask)
    #     return modulated_deform_conv(x, offset, mask, self.weight, self.bias, self.stride, self.padding, self.dilation,
    #                                  self.groups, self.deformable_groups)


def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    # From: https://github.com/rwightman/pypaddle-image-models/blob/master/timm/models/layers/weight_init.py
...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    From: https://github.com/rwightman/pypaddle-image-models/blob/master/timm/models/layers/drop.py
    """
...
def flow_warp(x, flow, interp_mode='bilinear', padding_mode='zeros', align_corners=True, use_pad_mask=False):
    ...
    Args:
        ...
        padding_mode (str): 'zeros' or 'border' or 'reflection'.
            Default: 'zeros'.
        align_corners (bool): Before pypaddle 1.3, the default value is
            align_corners=True. After pypaddle 1.3, the default value is
    ...
    n, _, h, w = x.size()
    # create mesh grid
    # grid_y, grid_x = paddle.meshgrid(paddle.arange(0, h).type_as(x), paddle.arange(0, w).type_as(x)) # an illegal memory access on TITAN RTX + Pypaddle1.9.1
    grid_y, grid_x = paddle.meshgrid(paddle.arange(0, h, dtype=x.dtype, device=x.device), paddle.arange(0, w, dtype=x.dtype, device=x.device))
    grid = paddle.stack((grid_x, grid_y), 2).float()  # W(x), H(y), 2
    ...
    if interp_mode == 'nearest4': # todo: bug, no gradient for flow model in this case!!! but the result is good
        vgrid_x_floor = 2.0 * paddle.floor(vgrid[:, :, :, 0]) / max(w - 1, 1) - 1.0
        vgrid_x_ceil = 2.0 * paddle.ceil(vgrid[:, :, :, 0]) / max(w - 1, 1) - 1.0
        vgrid_y_floor = 2.0 * paddle.floor(vgrid[:, :, :, 1]) / max(h - 1, 1) - 1.0
        vgrid_y_ceil = 2.0 * paddle.ceil(vgrid[:, :, :, 1]) / max(h - 1, 1) - 1.0

        output00 = F.grid_sample(x, paddle.stack((vgrid_x_floor, vgrid_y_floor), dim=3), mode='nearest', padding_mode=padding_mode, align_corners=align_corners)
        output01 = F.grid_sample(x, paddle.stack((vgrid_x_floor, vgrid_y_ceil), dim=3), mode='nearest', padding_mode=padding_mode, align_corners=align_corners)
        output10 = F.grid_sample(x, paddle.stack((vgrid_x_ceil, vgrid_y_floor), dim=3), mode='nearest', padding_mode=padding_mode, align_corners=align_corners)
        output11 = F.grid_sample(x, paddle.stack((vgrid_x_ceil, vgrid_y_ceil), dim=3), mode='nearest', padding_mode=padding_mode, align_corners=align_corners)

        return paddle.concat([output00, output01, output10, output11], 1)
...
class DCNv2PackFlowGuided(ModulatedDeformConvPack):
    ...
    def forward(self, x, x_flow_warpeds, x_current, flows):
        out = self.conv_offset(paddle.concat(x_flow_warpeds + [x_current] + flows, dim=1))
        o1, o2, mask = paddle.chunk(out, 3, axis=1)

        # offset
        offset = self.max_residue_magnitude * paddle.tanh(paddle.cat((o1, o2), dim=1))
        if self.pa_frames == 2:
            offset = offset + flows[0].flip(1).repeat(1, offset.size(1)//2, 1, 1)
        elif self.pa_frames == 4:
            offset1, offset2 = paddle.chunk(offset, 2, axis=1)
            offset1 = offset1 + flows[0].flip(1).repeat(1, offset1.size(1) // 2, 1, 1)
            offset2 = offset2 + flows[1].flip(1).repeat(1, offset2.size(1) // 2, 1, 1)
            offset = paddle.concat([offset1, offset2], dim=1)
        elif self.pa_frames == 6:
            offset = self.max_residue_magnitude * paddle.tanh(paddle.cat((o1, o2), dim=1))
            offset1, offset2, offset3 = paddle.chunk(offset, 3, dim=1)
            offset1 = offset1 + flows[0].flip(1).repeat(1, offset1.size(1) // 2, 1, 1)
            offset2 = offset2 + flows[1].flip(1).repeat(1, offset2.size(1) // 2, 1, 1)
            offset3 = offset3 + flows[2].flip(1).repeat(1, offset3.size(1) // 2, 1, 1)
            offset = paddle.concat([offset1, offset2, offset3], axis=1)

        # mask
        mask = paddle.nn.functional.sigmoid(mask)

        return paddle.vision.ops.deform_conv2d(x, offset, self.weight, self.bias, self.stride, self.padding,
                                         self.dilation, mask)
...
class BasicModule(nn.Layer):
    ...
    def __init__(self):
        ...
        self.basic_module = nn.Sequential(
            nn.Conv2D(in_channels=8, out_channels=32, kernel_size=7, stride=1, padding=3), nn.ReLU(),
            nn.Conv2D(in_channels=32, out_channels=64, kernel_size=7, stride=1, padding=3), nn.ReLU(),
            nn.Conv2D(in_channels=64, out_channels=32, kernel_size=7, stride=1, padding=3), nn.ReLU(),
            nn.Conv2D(in_channels=32, out_channels=16, kernel_size=7, stride=1, padding=3), nn.ReLU(),
...
class SpyNet(nn.Layer):
    ...
    def __init__(self, load_path=None, return_levels=[5]):
        ...
        self.basic_module = nn.LayerList([BasicModule() for _ in range(6)])
        if load_path:
            ...
            self.set_state_dict(paddle.load(load_path, map_location=lambda storage, loc: storage)['params'])

        self.register_buffer('mean', paddle.Tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1))
        self.register_buffer('std', paddle.Tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1))
    ...
    def process(self, ref, supp, w, h, w_floor, h_floor):
        ...
        for level in range(len(ref)):
            ...
            flow = self.basic_module[level](paddle.concat([
                ref[level],
                flow_warp(
            ...
            if level in self.return_levels:
                ...
                flow_out = F.interpolate(flow, size=(h//scale, w//scale), mode='bilinear', align_corners=False)
    ...
    def forward(self, ref, supp):
        ...
        ref = F.interpolate(ref, size=(h_floor, w_floor), mode='bilinear', align_corners=False)
        supp = F.interpolate(supp, size=(h_floor, w_floor), mode='bilinear', align_corners=False)
...
class Upsample(nn.Sequential):
    ...
    def __init__(self, scale, num_feat):
        assert LooseVersion(paddle.__version__) >= LooseVersion('1.8.1'), \
            'Pypaddle version >= 1.8.1 to support 5D PixelShuffle.'

        class Transpose_Dim12(nn.Layer):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            ...
            m.append(nn.Conv3(num_feat, 9 * num_feat, kernel_size=(1, 3, 3), padding=(0, 1, 1)))
            m.append(Transpose_Dim12())
...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, mut_attn=True):
        ...
        self.relative_position_bias_table = nn.Parameter(
            paddle.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] - 1),
        ...
        if self.mut_attn:
            qkv = self.qkv_mut(x + self.position_bias.repeat(1, 2, 1)).reshape(B_, N, 3, self.num_heads,
                                                                               C // self.num_heads).permute(2, 0, 3, 1,
                                                                                                            4)
            (q1, q2), (k1, k2), (v1, v2) = paddle.chunk(qkv[0], 2, dim=2), paddle.chunk(qkv[1], 2, dim=2), paddle.chunk(
            ...
            x_out = paddle.cat([paddle.cat([x1_aligned, x2_aligned], 1), x_out], 2)
    ...
    def get_position_index(self, window_size):
        ...
        coords_d = paddle.arange(window_size[0])
        coords_h = paddle.arange(window_size[1])
        coords_w = paddle.arange(window_size[2])
        coords = paddle.stack(paddle.meshgrid(coords_d, coords_h, coords_w))  # 3, Wd, Wh, Ww
    ...
    def get_sine_position_encoding(self, HW, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        not_mask = paddle.ones([1, HW[0], HW[1]])
        ...
        pos_x = paddle.stack((pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4).flatten(3)
        pos_y = paddle.stack((pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), dim=4).flatten(3)
        pos_embed = paddle.cat((pos_y, pos_x), dim=3).permute(0, 3, 1, 2)
...
class TMSA(nn.Layer):
    ...
    Args:
        ...
        use_checkpoint_attn (bool): If True, use paddle.checkpoint for attention modules. Default: False.
        use_checkpoint_ffn (bool): If True, use paddle.checkpoint for feed-forward modules. Default: False.
    """
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if any(i > 0 for i in shift_size):
            shifted_x = paddle.roll(x, shifts=(-shift_size[0], -shift_size[1], -shift_size[2]), dims=(1, 2, 3))
        ...
        if any(i > 0 for i in shift_size):
            x = paddle.roll(shifted_x, shifts=(shift_size[0], shift_size[1], shift_size[2]), dims=(1, 2, 3))
        else:
            x = shifted_x
...
class Stage(nn.Layer):
    ...
    def forward(self, x, flows_backward, flows_forward):
        ...
        x = self.pa_fuse(paddle.cat([x, x_backward, x_forward], 2).permute(0, 1, 3, 4, 2)).permute(0, 4, 1, 2, 3)
    ...
    def get_aligned_feature_2frames(self, x, flows_backward, flows_forward):
        ...
        # backward
        n = x.size(1)
        x_backward = [paddle.zeros_like(x[:, -1, ...])]
        ...
        # forward
        x_forward = [paddle.zeros_like(x[:, 0, ...])]
        ...
        return [paddle.stack(x_backward, 1), paddle.stack(x_forward, 1)]
    ...
    def get_aligned_feature_4frames(self, x, flows_backward, flows_forward):
        ...
        for i in range(n, 1, -1):
            ...
            if i == n:
                x_ii = paddle.zeros_like(x[:, n - 2, ...])
                flow2 = paddle.zeros_like(flows_backward[1][:, n - 3, ...])
            else:
                x_ii = x[:, i, ...]
            ...
            x_backward.insert(0,
                self.pa_deform(paddle.cat([x_i, x_ii], 1), [x_i_warped, x_ii_warped], x[:, i - 2, ...], [flow1, flow2]))
        ...
        for i in range(-1, n - 2):
            ...
            if i == -1:
                x_ii = paddle.zeros_like(x[:, 1, ...])
                flow2 = paddle.zeros_like(flows_forward[1][:, 0, ...])
            else:
                x_ii = x[:, i, ...]
    ...
    def get_aligned_feature_6frames(self, x, flows_backward, flows_forward):
        ...
        for i in range(n + 1, 2, -1):
            ...
            if i == n + 1:
                x_ii = paddle.zeros_like(x[:, -1, ...])
                flow2 = paddle.zeros_like(flows_backward[1][:, -1, ...])
                x_iii = paddle.zeros_like(x[:, -1, ...])
                flow3 = paddle.zeros_like(flows_backward[2][:, -1, ...])
            elif i == n:
                x_ii = x[:, i - 1, ...]
            ...
            x_backward.insert(0,
                              self.pa_deform(paddle.cat([x_i, x_ii, x_iii], 1), [x_i_warped, x_ii_warped, x_iii_warped],
        ...
        for i in range(0, n - 1):
            ...
            if i == 0:
                x_ii = paddle.zeros_like(x[:, 0, ...])
                flow2 = paddle.zeros_like(flows_forward[1][:, 0, ...])
                x_iii = paddle.zeros_like(x[:, 0, ...])
                flow3 = paddle.zeros_like(flows_forward[2][:, 0, ...])
            elif i == 1:
                x_ii = x[:, i - 1, ...]
            ...
            x_ii_warped = flow_warp(x_ii, flow2.permute(0, 2, 3, 1), 'bilinear')  # frame i-2 aligned towards i
            x_iii_warped = flow_warp(x_iii, flow3.permute(0, 2, 3, 1), 'bilinear')  # frame i-3 aligned towards i
            x_forward.append(self.pa_deform(paddle.cat([x_i, x_ii, x_iii], 1), [x_i_warped, x_ii_warped, x_iii_warped],
...
class VRT(nn.Layer):
    """ Video Restoration Transformer (VRT).
        A Pypaddle impl of : `VRT: A Video Restoration Transformer`  -
    ...
    Args:
        ...
        use_checkpoint_attn (bool): If True, use paddle.checkpoint for attention modules. Default: False.
        use_checkpoint_ffn (bool): If True, use paddle.checkpoint for feed-forward modules. Default: False.
        no_checkpoint_attn_blocks (list[int]): Layers without paddle.checkpoint for attention modules.
        no_checkpoint_ffn_blocks (list[int]): Layers without paddle.checkpoint for feed-forward modules.
    """
    ...
    def forward(self, x):
        ...
        x = paddle.cat([x, x_backward, x_forward], 2)

        # concatenate noise level map
        if self.nonblind_denoising:
            x = paddle.cat([x, noise_level_map], 2)
        ...
        if self.upscale == 1:
            ...
            _, _, C, H, W = x.shape
            return x + paddle.nn.functional.interpolate(x_lq, size=(C, H, W), mode='trilinear', align_corners=False)
    ...
    def get_flow_4frames(self, flows_forward, flows_backward):
        ...
        for flows in flows_backward:
            ...
            flows_backward2.append(paddle.stack(flow_list, 1))
        ...
        for flows in flows_forward:
            ...
            flows_forward2.append(paddle.stack(flow_list, 1))
    ...
    def get_flow_6frames(self, flows_forward, flows_backward, flows_forward2, flows_backward2):
        ...
        for flows, flows2 in zip(flows_backward, flows_backward2):
            ...
            flows_backward3.append(paddle.stack(flow_list, 1))
        ...
        for flows, flows2 in zip(flows_forward, flows_forward2):
            ...
            flows_forward3.append(paddle.stack(flow_list, 1))
    ...
    def get_aligned_image_2frames(self, x, flows_backward, flows_forward):
        ...
        # backward
        n = x.size(1)
        x_backward = [paddle.zeros_like(x[:, -1, ...]).repeat(1, 4, 1, 1)]
        for i in range(n - 1, 0, -1):
            x_i = x[:, i, ...]
        ...
        # forward
        x_forward = [paddle.zeros_like(x[:, 0, ...]).repeat(1, 4, 1, 1)]
        for i in range(0, n - 1):
            x_i = x[:, i, ...]
...
if __name__ == '__main__':
    ...
    x = paddle.randn((2, 12, 3, height, width)).to(device)
    x = model(x)
    print(x.shape)
