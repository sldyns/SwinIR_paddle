...
class VGGFeatureExtractor(nn.Layer):
    def __init__(self, feature_layer=[2,7,16,25,34], use_input_norm=True, use_range_norm=False):
        ...
        if self.use_input_norm:
            ...
            std = paddle.Tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
...
class TVLoss(nn.Layer):
    def __init__(self, tv_loss_weight=1):
        ...
        https://github.com/jxgu1016/Total_Variation_Loss.pypaddle
        Args:
            tv_loss_weight (int):
        """
...
def g_path_regularize(fake_img, latents, mean_path_length, decay=0.01):
    noise = paddle.randn_like(fake_img) / math.sqrt(
...
def gradient_penalty_loss(discriminator, real_data, fake_data, weight=None):
    ...
    Args:
        discriminator (nn.Layer): Network for the discriminator.
    ...
    alpha = real_data.new_tensor(paddle.rand(batch_size, 1, 1, 1))
