...
class ModelVRT(ModelPlain):
    ...
    def test(self):
        ...
        if pad_seq:
            n = n + 1
            self.L = paddle.cat([self.L, self.L[:, -1:, :, :, :]], dim=1)

        if flip_seq:
            self.L = paddle.cat([self.L, self.L.flip(1)], dim=1)
    ...
    def _test_video(self, lq):
        ...
        if num_frame_testing:
            ...
            E = paddle.zeros(b, d, c, h*sf, w*sf)
            W = paddle.zeros(b, d, 1, 1, 1)

            for d_idx in d_idx_list:
                lq_clip = lq[:, d_idx:d_idx+num_frame_testing, ...]
                out_clip = self._test_clip(lq_clip)
                out_clip_mask = paddle.ones((b, min(num_frame_testing, d), 1, 1, 1))
            ...
            lq = paddle.cat([lq, paddle.flip(lq[:, -d_pad:, ...], [1])], 1)
    ...
    def _test_clip(self, lq):
        ...
        if size_patch_testing:
            ...
            lq = paddle.cat([lq, paddle.flip(lq[:, :, :, -h_pad:, :], [3])], 3)
            lq = paddle.cat([lq, paddle.flip(lq[:, :, :, :, -w_pad:], [4])], 4)
