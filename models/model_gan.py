...
class ModelGAN(ModelBase):
    ...
    def optimize_parameters(self, current_step):
        ...
        if current_step % self.D_update_ratio == 0 and current_step > self.D_init_iters:  # updata D first
            ...
            if self.opt['train']['gan_type'] in ['gan', 'lsgan', 'wgan', 'softplusgan']:
                ...
                D_loss = self.D_lossfn_weight * (
                        self.D_lossfn(pred_d_real - paddle.mean(pred_g_fake, 0, True), False) +
                        self.D_lossfn(pred_g_fake - paddle.mean(pred_d_real, 0, True), True)) / 2
        ...
        if self.opt_train['gan_type'] in ['gan', 'lsgan', 'wgan', 'softplusgan']:
            ...
            l_d_real = 0.5 * self.D_lossfn(pred_d_real - paddle.mean(pred_d_fake, 0, True), True)
            l_d_real.backward()
            # fake
            pred_d_fake = self.netD(self.E.detach())
            l_d_fake = 0.5 * self.D_lossfn(pred_d_fake - paddle.mean(pred_d_real.detach(), 0, True), False)
            l_d_fake.backward()
        ...
        self.log_dict['D_real'] = paddle.mean(pred_d_real.detach())
        self.log_dict['D_fake'] = paddle.mean(pred_d_fake.detach())
