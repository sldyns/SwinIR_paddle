...
class FFDNet(nn.Layer):
    ...
    def forward(self, x, sigma):
        ...
        x = paddle.nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))(x)

        x = self.m_down(x)
        # m = paddle.ones(sigma.size()[0], sigma.size()[1], x.size()[-2], x.size()[-1]).type_as(x).mul(sigma)
...
if __name__ == '__main__':
    ...
    x = paddle.randn((2,1,240,240))
    sigma = paddle.randn(2,1,1,1)
    x = model(x, sigma)
    print(x.shape)
