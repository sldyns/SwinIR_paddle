...
class PixelNorm(nn.Layer):
    ...
    def forward(self, input):
        return input * paddle.rsqrt(paddle.mean(input ** 2, dim=1, keepdim=True) + 1e-8)


def make_kernel(k):
    k = paddle.tensor(k, dtype=paddle.float32)
...
class EqualConv2D(nn.Layer):
    def __init__(
        ...
        self.weight = nn.Parameter(
            paddle.randn(out_channel, in_channel, kernel_size, kernel_size)
        )
        ...
        if bias:
            self.bias = nn.Parameter(paddle.zeros(out_channel))
...
class EqualLinear(nn.Layer):
    def __init__(
        ...
        self.weight = nn.Parameter(paddle.randn(out_dim, in_dim).div_(lr_mul))

        if bias:
            self.bias = nn.Parameter(paddle.zeros(out_dim).fill_(bias_init))
...
class ModulatedConv2D(nn.Layer):
    def __init__(
        ...
        self.weight = nn.Parameter(
            paddle.randn(1, out_channel, in_channel, kernel_size, kernel_size)
        )
...
class NoiseInjection(nn.Layer):
    def __init__(self):
        ...
        self.weight = nn.Parameter(paddle.zeros(1))

    def forward(self, image, noise=None):
        
        if noise is not None:
            #print(image.shape, noise.shape)
            if isconcat: return paddle.cat((image, self.weight * noise), dim=1) # concat
        ...
        return image + self.weight * noise
        #return paddle.cat((image, self.weight * noise), dim=1)
...
class ConstantInput(nn.Layer):
    def __init__(self, channel, size=4):
        ...
        self.input = nn.Parameter(paddle.randn(1, channel, size, size))
...
class ToRGB(nn.Layer):
    def __init__(self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        ...
        self.bias = nn.Parameter(paddle.zeros(1, 3, 1, 1))
    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(self.n_mlp + 1):
                size = 2 ** (i+2)
                noise.append(paddle.randn(batch, self.channels[size], size, size, device=styles[0].device))
        ...
        if len(styles) < 2:
            ...
            latent = paddle.cat([latent, latent2], 1)
