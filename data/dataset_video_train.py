...
class VideoRecurrentTrainDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_gts = paddle.stack(img_results[len(img_lqs) // 2:], dim=0)
        img_lqs = paddle.stack(img_results[:len(img_lqs) // 2], dim=0)
...
class VideoRecurrentTrainNonblindDenoisingDataset(VideoRecurrentTrainDataset):
    ...
    def __getitem__(self, index):
        ...
        img_gts = paddle.stack(img_gts, dim=0)

        # we add noise in the network
        noise_level = paddle.empty((1, 1, 1, 1)).uniform_(self.sigma_min, self.sigma_max)
        noise = paddle.normal(mean=0, std=noise_level.expand_as(img_gts))
        img_lqs = img_gts + noise

        t, _, h, w = img_lqs.shape
        img_lqs = paddle.cat([img_lqs, noise_level.expand(t, 1, h, w)], 1)
...
class VideoRecurrentTrainVimeoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_results = utils_video.img2tensor(img_results)
        img_lqs = paddle.stack(img_results[:7], dim=0)
        img_gts = paddle.stack(img_results[7:], dim=0)

        if self.flip_sequence:  # flip the sequence: 7 frames to 14 frames
            img_lqs = paddle.cat([img_lqs, img_lqs.flip(0)], dim=0)
            img_gts = paddle.cat([img_gts, img_gts.flip(0)], dim=0)
        elif self.pad_sequence:  # pad the sequence: 7 frames to 8 frames
            img_lqs = paddle.cat([img_lqs, img_lqs[-1:,...]], dim=0)
            img_gts = paddle.cat([img_gts, img_gts[-1:,...]], dim=0)
