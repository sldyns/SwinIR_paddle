...
class VideoRecurrentTestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # for non-blind video denoising
            ...
            noise_level = paddle.ones((1, 1, 1, 1)) * self.sigma
            noise = paddle.normal(mean=0, std=noise_level.expand_as(imgs_gt))
            imgs_lq = imgs_gt + noise
            t, _, h, w = imgs_lq.shape
            imgs_lq = paddle.cat([imgs_lq, noise_level.expand(t, 1, h, w)], 1)
        else:
...
class VideoTestVimeo90KDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.pad_sequence:  # pad the sequence: 7 frames to 8 frames
            imgs_lq = paddle.cat([imgs_lq, imgs_lq[-1:,...]], dim=0)
