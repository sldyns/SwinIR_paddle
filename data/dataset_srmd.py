...
class DatasetSRMD(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # -------------------------------------
        self.p = hdf5storage.loadmat(os.path.join('kernels', 'srmd_pca_pypaddle.mat'))['p']
    ...
    def __getitem__(self, index):
        ...
        k_reduced = paddle.from_numpy(k_reduced).float()
        ...
        if self.opt['phase'] == 'train':
            ...
            if random.random() < 0.1:
                noise_level = paddle.zeros(1).float()
            else:
                noise_level = paddle.FloatTensor([np.random.uniform(self.sigma_min, self.sigma_max)])/255.0
                # noise_level = paddle.rand(1)*50/255.0
                # noise_level = paddle.min(paddle.from_numpy(np.float32([7*np.random.chisquare(2.5)/255.0])),paddle.Tensor([50./255.]))
    
        else:

            img_H, img_L = util.single2tensor3(img_H), util.single2tensor3(img_L)
            noise_level = noise_level = paddle.FloatTensor([self.sigma_test])

        # ------------------------------------
        # add noise
        # ------------------------------------
        noise = paddle.randn(img_L.size()).mul_(noise_level).float()
        img_L.add_(noise)

        # ------------------------------------
        # get degradation map M
        # ------------------------------------
        M_vector = paddle.cat((k_reduced, noise_level), 0).unsqueeze(1).unsqueeze(1)
