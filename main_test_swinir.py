...
def main():
    ...
    parser.add_argument('--model_path', type=str,
                        default='./test_tipc/output/swinir_denoising_color_15/models/110_G.pdparams')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        img_lq = paddle.Tensor(img_lq).astype(paddle.float32).unsqueeze(0)  # CHW-RGB to NCHW-RGB
        ...
        # save image
        output = paddle.clip(output,0, 1).squeeze().numpy()
        if output.ndim == 3:
